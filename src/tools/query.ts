...
import { validateIdentifier, validateQueryFieldToken, validateSafeSoqlFragment } from "../utils/sanitize.js";
...
export async function handleQueryRecords(conn: any, args: QueryArgs) {
  ...
  try {
    ...
    if (whereClause) {
      const wv = validateSafeSoqlFragment(whereClause);
      if (!wv.valid) {
        return { content: [{ type: "text", text: `Invalid WHERE clause: ${wv.error}` }], isError: true };
      }
    }
    if (orderBy) {
      const ov = validateSafeSoqlFragment(orderBy);
      if (!ov.valid) {
        return { content: [{ type: "text", text: `Invalid ORDER BY clause: ${ov.error}` }], isError: true };
      }
    }
